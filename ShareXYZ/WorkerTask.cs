...
using ShareXYZ.UploadersLib.FileUploaders;
using ShareXYZ.UploadersLib.GUI;
using ShareXYZ.UploadersLib.HelperClasses;
using ShareXYZ.UploadersLib.ImageUploaders;
using ShareXYZ.UploadersLib.TextUploaders;
using ShareXYZ.UploadersLib.URLShorteners;
using System;
    ...
    public class WorkerTask : IDisposable
    {
        ...
        private void DoUploadJob()
        {
            if (Info.IsUploadJob)
            {
                if (Program.Settings.ShowUploadWarning && MessageBox.Show(
                    ...
                    "ShareXYZ - " + Resources.UploadTask_DoUploadJob_First_time_upload_warning,
                ...
                if (Program.Settings.LargeFileSizeWarning > 0)
                {
                    ...
                    {
                        using (MyMessageBox msgbox = new MyMessageBox(Resources.UploadTask_DoUploadJob_You_are_attempting_to_upload_a_large_file, "ShareXYZ",
            ...
            if (!string.IsNullOrEmpty(url))
            {
                ...
                switch (Info.TaskSettings.URLSharingServiceDestination)
                {
                    ...
                    case URLSharingServices.Pushbullet:
                        new Pushbullet(Program.UploadersConfig.PushbulletSettings).PushLink(url, "ShareXYZ: URL Share");
                        break;
                }
            }
        }
        ...
        private bool DownloadAndUpload()
        {
            ...
            if (!string.IsNullOrEmpty(Info.FilePath))
            {
                ...
                {
                    DebugHelper.WriteException(e);
                    MessageBox.Show(string.Format(Resources.UploadManager_DownloadAndUploadFile_Download_failed, e), "ShareXYZ", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        ...
        private bool LoadFileStream()
        {
            ...
            {
                MessageBox.Show("ShareXYZ - " + Resources.TaskManager_task_UploadCompleted_Error, e.Message, MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
