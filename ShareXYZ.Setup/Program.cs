...
namespace ShareXYZ.Setup
{
    internal class Program
    {
        private enum SetupType
        {
            ...
            Beta // Build setup + upload it using "Debug/ShareXYZ.exe"
        }
        ...
        private static string debugPath = Path.Combine(debugDir, "ShareXYZ.exe");
        ...
        private static string innoSetupScriptPath = Path.Combine(parentDir, @"InnoSetup\ShareXYZ setup.iss");
        ...
        private static void CreatePortable()
        {
            ...
            string[] endsWith = new string[] { "ShareXYZ.exe", "ShareXYZ.exe.config", ".dll", ".css", ".txt" };
            ...
            CopyFile(Path.Combine(parentDir, @"..\ShareXYZ_Chrome\ShareXYZ_Chrome\bin\Release\ShareXYZ_Chrome.exe"), portableDir);

            File.WriteAllText(Path.Combine(portableDir, "PersonalPath.cfg"), "ShareX", Encoding.UTF8);

            //FileVersionInfo versionInfo = FileVersionInfo.GetVersionInfo(Path.Combine(releaseDir, "ShareXYZ.exe"));
