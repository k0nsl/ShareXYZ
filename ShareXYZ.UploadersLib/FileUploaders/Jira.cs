...
namespace ShareXYZ.UploadersLib.FileUploaders
{
    public class Jira : FileUploader, IOAuth
    {
        ...
        static Jira()
        {
            ...
            // makecert -pe -n "CN=ShareX" -a sha1 -sky exchange -sp "Microsoft RSA SChannel Cryptographic Provider" -sy 12 -len 1024 -sv jira_ShareXYZ.pvk jira_ShareXYZ.cer
            // pvk2pfx -pvk jira_ShareXYZ.pvk -spc jira_ShareXYZ.cer -pfx jira_ShareXYZ.pfx
            // (Based on: http://nick-howard.blogspot.fr/2011/05/makecert-x509-certificates-and-rsa.html)
            using (Stream stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("ShareXYZ.UploadersLib.APIKeys.jira_ShareXYZ.pfx"))
            {
